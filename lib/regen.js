...
function regen(reg) {
	...
	while (builder.i < reg.length) {
		...
		_patterns.push(reg.substring(i + 1, end));
		builder.i = end + 1;
		...
		if (parseInt(reg[i]) - 1 < _patterns.length) {
		    var sub = _patterns[parseInt(reg[i]) - 1];
		    var reg_repeat = parseRepeat(builder, builder.reg.substr(i + 1));
		    if (reg_repeat > 0) {
			for (var it = 0; it < reg_repeat; it++)
			    builder.out += parseRegex(sub);
		    } else
			builder.out += strRepeat(parseRegex(sub), -reg_repeat);
		}
		continue;
	    }
